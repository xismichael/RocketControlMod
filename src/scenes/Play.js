class Play extends Phaser.Scene {
    ...
    create() {
        ...
        // add Spaceships (x3)
        this.ship01 = new Spaceship(this, game.config.width + borderUISize * 6, borderUISize * 4, 'spaceship', 0, 30, Math.random() < 0.5).setOrigin(0, 0);
        this.ship02 = new Spaceship(this, game.config.width + borderUISize * 3, borderUISize * 5 + borderPadding * 2, 'spaceship', 0, 20, Math.random() < 0.5).setOrigin(0, 0);
        this.ship03 = new Spaceship(this, game.config.width, borderUISize * 6 + borderPadding * 4, 'spaceship', 0, 10, Math.random() < 0.5).setOrigin(0, 0);
    ...
    update() {
        ...
        if (this.timeLeft < (this.time.now - this.sceneStartTime) / 1000) {
            this.timeLeftText.text = `TIME REMAINING: 0`;
            this.gameOver = true;
            let endConfig = {
                fontFamily: 'Courier',
                fontSize: '28px',
        ...
        if (!this.gameOver) {
            this.timeLeftText.text = `TIME REMAINING: ${Math.ceil(this.timeLeft - ((this.time.now - this.sceneStartTime) / 1000))}`;
        }
